    ...
    def get_serializer_context(self):
        ...
        if triagem_id:
            try:
                ...
                raise ValidationError("Triagem não encontrada")
        return context

    def get_queryset(self):
     user = self.request.user

    # Para enfermeiros: retorna todos os agendamentos confirmados
     if hasattr(user, 'enfermeiro'):
        return Agendamento.objects.filter(triagem_IA__status_agendamento='CONFIRMADO').order_by('data_hora_consulta')

    # Para pacientes: retorna apenas os próprios agendamentos confirmados
     elif hasattr(user, 'paciente'):
        return Agendamento.objects.filter(
            triagem_IA__ficha_medica_paciente__paciente=user.paciente,
            triagem_IA__status_agendamento='CONFIRMADO'
        ).order_by('data_hora_consulta')

    # Para médicos: retorna agendamentos em que ele é o médico e confirmados
