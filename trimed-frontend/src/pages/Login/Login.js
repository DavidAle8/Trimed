...
const Login = () => {
  ...
  const verificarCredenciais = async ({ email, senha }) => {
    ...
    const response = await fetch(URL_API, {
      ...
      body: JSON.stringify({ email, password: senha }) // campo password
    });
  ...
  // Busca dados do usuário autenticado em vários endpoints com header Authorization
  const buscarDadosUsuario = async (token) => {
    ...
    for (let ep of endpoints) {
      try {
        ...
        // Garanta que o nome está sendo retornado corretamente
        return { 
          nome: data.nome || data.nome_completo || data.username || 'Usuário',
          email: data.email,
          tipo: ep.tipo,
          ...data
        };
        ...
        console.warn(`Não conseguiu buscar ${ep.tipo}:`, e);
      }
    }
    return null;
  };
  ...
  const aoSubmeter = async (e) => {
    ...
    try {
      ...
      const usuario = await buscarDadosUsuario(tokens.access);

      if (!usuario) throw new Error("Não foi possível identificar o tipo de usuário.");
      ...
      setErro({ mensagem: erroLogin.message, tipo: 'credenciais' });
    } finally {
      setCarregando(false);
    }
  };
